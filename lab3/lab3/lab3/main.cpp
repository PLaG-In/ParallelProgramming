...
void draw_3d_model_simple(ModelPtrArray const& models, TGAImage &frame, float *zbuffer)
{
    ...
    for (auto const& pModel : models) {
		threadPool.RunAsync(&draw_3d_model_tile, std::ref(*pModel), std::ref(tile1));
		threadPool.RunAsync(&draw_3d_model_tile, std::ref(*pModel), std::ref(tile2));
		threadPool.RunAsync(&draw_3d_model_tile, std::ref(*pModel), std::ref(tile3));
		threadPool.RunAsync(&draw_3d_model_tile, std::ref(*pModel), std::ref(tile4));
